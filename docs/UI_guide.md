...
- 접근성 처리는 반드시 `useModalAccessibility(modalRef, onClose)` 훅을 사용한다 (`shared/hooks/useModalAccessibility.ts`)
...
> **클릭·탭 가능한 모든 인터랙티브 요소는 반드시 `cursor-pointer`를 명시한다.**
> 브라우저 기본값에 의존하지 않는다.

| 상태 | cursor 값 | 적용 대상 |
|---|---|---|
| 기본 인터랙티브 | `pointer` | 버튼(`<button>`), 링크(`<a>`), 체크박스·라디오 label, select, 탭, 카드(클릭형), Pill 선택 label, 토글, 페이지네이션 등 클릭·탭 가능한 모든 요소 |
...
- 체크박스·라디오는 sr-only input이 아닌 **감싸는 `<label>`** 에 `cursor-pointer` 적용
